...
export const useAI = () => {
  ...
  const generatePlaylist = async (prompt: string, songCount: number = 15): Promise<PlaylistResponse | null> => {
    ...
    // Extract energy, tempo, and diversity parameters if they are included in the prompt
    ...
    // Extract whether to include obscure tracks
    const includeObscure = prompt.includes("Include some obscure/lesser-known tracks");
    
    // Construct a more detailed system prompt based on the extracted parameters
    const systemPrompt = `
    ...
    try {
      ...
      // Apply smart post-processing to improve the playlist
      if (parsedResponse && Array.isArray(parsedResponse.playlist)) {
        // If user wanted diversity but we have duplicate artists, we could filter here
        if (diversityMatch && parseInt(diversityMatch[1]) > 7) {
          ...
          // If we have enough tracks after filtering
