...
export const useSpotify = (token: string | null) => {
  ...
  const uploadCoverImage = useCallback(
    async (playlistId: string, image: ProcessedImage): Promise<boolean> => {
      if (!token) return false;
      
      try {
        const response = await fetch(`${SPOTIFY_API_URL}/playlists/${playlistId}/images`, {
          method: "PUT",
          headers: {
  ...
  const createSpotifyPlaylist = useCallback(
    async (
      ...
      coverImage: ProcessedImage | null = null
    ): Promise<string | null> => {
      if (!token) {
        console.error("Missing Spotify Token!");
      ...
      try {
        ...
        if (coverImage) {
          const imageUploaded = await uploadCoverImage(playlistId, coverImage);
          if (!imageUploaded) {
            console.warn("Failed to upload cover image, but playlist was created successfully");
          }
        }
